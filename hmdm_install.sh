#!/bin/bash
#
# MDM-Lab installer script
# Tested on Ubuntu Linux 18.04 - 24.04, Ubuntu 22.04 is recommended
#
REPOSITORY_BASE=https://laborato.org/files
CLIENT_VERSION=5.19
DEFAULT_SQL_HOST=95.142.43.61
DEFAULT_SQL_PORT=5436
DEFAULT_SQL_BASE=hmdm
DEFAULT_SQL_USER=hmdm
DEFAULT_SQL_PASS=123456password
...
DEFAULT_BASE_DOMAIN=95.142.43.61
...
if [ ! -f $SERVER_WAR ]; then
    echo "FAILED to find the WAR file of MDM!"
...
if [ ! -d "$TOMCAT_HOME" ]; then
    ...
    if [ ! -d "$TOMCAT_HOME" ]; then
        ...
        echo "MDM-Lab installer requires this directory to install the WAR file!"
        exit 1
    fi
fi
...
if [ ! -z "$TABLE_EXISTS" ]; then
    ...
    echo "To re-deploy MDM-Lab, the database needs to be cleared."
	...
	echo "Database has been cleared."
        ...
        echo "MDM-Lab installation aborted"
	exit 1
    fi
fi
...
echo "=================="
echo "Please choose where the files uploaded to MDM-Lab will be stored"
echo "If the directory doesn't exist, it will be created"
echo "##### FOR TOMCAT 9, USE SANDBOXED DIR: /var/lib/tomcat9/work #####"
echo

read -e -p "MDM-Lab storage directory [$DEFAULT_LOCATION]: " -i "$DEFAULT_LOCATION" LOCATION
...
echo
read -e -p "MDM-Lab scripts directory [$DEFAULT_SCRIPT_LOCATION]: " -i "$DEFAULT_SCRIPT_LOCATION" SCRIPT_LOCATION
...
echo "====================="
echo "MDM-Lab requires access from Internet"
...
echo
echo "To enable password recovery function, MDM-Lab must be connected to SMTP."
...
echo
echo "======================================"
echo "Minimal installation of MDM-Lab has been done!"
...
if [[ "$REPLY" =~ ^[Yy]$ ]]; then
    ...
    echo
    echo "======================================"
    echo "Secure installation of MDM-Lab has been done!"
    echo "At this step, you can open in your web browser:"
...
read -e -p "Move required APKs from laborato.org to your server [Y/n]?: " -i "Y" REPLY
if [[ "$REPLY" =~ ^[Yy]$ ]]; then
    ...
    echo "UPDATE applicationversions SET url=REPLACE(url, 'https://laborato.org', '$PROTOCOL://$BASE_HOST$BASE_PATH') WHERE url IS NOT NULL" | psql $PSQL_CONNSTRING >/dev/null 2>&1
    cd $CURRENT_DIR
fi

echo
echo "======================================"
echo "MDM-Lab installation is completed!"
