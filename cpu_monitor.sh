...
if [ $CPU_EXCEEDS -eq 1 ]; then
    ...
    echo "$DATETIME Abnormal CPU usage: $CPU_TOTAL" >> $ERROR_LOG_FILE
     ...
     # Prepare e-mail
    echo "MDM-Lab CPU abnormal usage detected: $CPU_TOTAL" > $TMP_BODY
	
	for MAIL in $MAILS
    do
        cat $TMP_BODY | mail -s "Problem at MDM-Lab server" $MAIL
    done
    rm $TMP_BODY
...
# Check if there was a recent error, then it's recovered
    ...
    then
        ...
        # Prepare e-mail
        echo "MDM-Lab server is recovered, CPU usage is normal: $CPU_TOTAL" > $TMP_BODY

        for MAIL in $MAILS
        do
            cat $TMP_BODY | mail -s "MDM-Lab server is recovered" $MAIL
        done
        rm $TMP_BODY
